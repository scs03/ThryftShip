...
const Trying = () => {
  ...
  const groupedProducts = formData.products.reduce((groups, product) => {
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <div className='w-full mt-4 min-h-full flex justify-center'>
        <div className='w-7/8 md:w-1/2 flex flex-col justify-center'>
          ...
          <div className='flex flex-col card p-4 0 w-full'>
            <h1 className='text-2xl font-bold w-full flex '>Order Placed!</h1>
            {Object.keys(groupedProducts).map((style, index) => (
              <div key={index} className='card mt-4 w-full border-2 p-4'>
                <p className='border-b-2'>Arriving: {style}</p>
